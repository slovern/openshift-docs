...
==== Updating managed clusters with Topology Aware Lifecycle Operator (Technology Preview)

You can now use the Topology Aware Lifecycle Operator to perform updates on multiple single node Openshift (SNO) clusters by using {rh-rhacm-first} policies. From {product-title} {product-version}, TALO is available in the OperatorHub. For more information, see xref:../scalability_and_performance/cnf-talo-for-cluster-upgrades.html#cnf-about-topology-aware-lifecycle-operator-config_cnf-topology-aware-lifecycle-operator[About the Topology Aware Lifecycle Operator configuration].
...
* You can use Topology Aware Lifecycle Operator (TALO) to orchestrate the application of the configuration CRs to the hub cluster.
...
|Topology Aware Lifecycle Operator (TALO)
|-
|-
|TP
...
[id="ocp-4-10-ran-ga-known-issues"]
* The Kubelet service monitor scrape interval is currently set to a hard-coded value. This means that there is less available CPU resources for workloads.
...
* Currently, you can add a `managedPolicy` in a `ClusterGroupUpgrade` manifest that is already enforced. This should not be possible.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2044304[*BZ#2044304*])

* If a `ClusterGroupUpgrade` CR has a `blockingCR` specified, and that `blockingCR` fails silently, for example if there is a typo in the list of clusters, the `ClusterGroupUpgrade` CR is applied even though the `blockingCR` is not applied in the cluster.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2042601[*BZ#2042601*])

* If a `ClusterGroupUpgrade` CR validation fails for any reason, for example, because of an invalid spoke name, no status is available for the `ClusterGroupUpgrade` CR, as if the CR is disabled.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2040828[*BZ#2040828*])

* Currently, for `ClusterGroupUpgrade` CR state changes, there is only a single condition type available - `Ready`. The `Ready` condition can have a status of `True` or `False` only. This does not reflect the range of states the `ClusterGroupUpgrade` CR can be in.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2042596[*BZ#2042596*])

* The alpha channel for the PTP Operator reports an error that prevents a `PtpConfig` CR from being applied when the `summary_interval` value does not match the value set for `logSyncInterval`. To work around this issue, update all reference PTP configurations to set `summary_interval` and `logSyncInterval` to -4.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2040799[*BZ#2040799*])

* When you create spoke clusters using policies and the OpenShift GitOps Operator v1.4.1 on a {rh-rhacm-first} hub cluster, clusters and policies apps report errors and do not recover.
+
To work around this issue, pin the OpenShift GitOps Operator to v1.3.2. To do this, make the following changes on the hub cluster:
+
. Apply the following `Subscription` CR to the hub cluster:
+
[source,yaml]
----
...
----
+
. Optional: Manually approve the `installplan` created for the subscription:
+
[source,terminal]
----
...
----
+
.. Edit the `installplan` that has CSV version `v.1.3.2`:
+
[source,terminal]
----
...
----
+
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2046775[*BZ#2046775*])

* Sometimes, when provisioning spoke clusters, ArgoCD applications with 100 or more `Siteconfig` CRs can cause `OOMKilled` memory errors on the hub cluster, even when the `openshift-gitops-application-controller-0` is allocated 40GB memory.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2026476[*BZ#2026476*])

* When deploying a multi-node cluster on bare-metal nodes, the machine config pool (MCP) adds an additional CRI-O drop-in that circumvents the link:https://github.com/openshift-kni/cnf-features-deploy/blob/master/cnf-tests/README.md#container-mount-namespace[container mount namespace drop-in]. This results in CRI-O being in the base namespace while the kubelet is in the hidden namespace. All containers fail to get any kubelet-mounted filesystems, such as secrets and tokens.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2028590[*BZ#2028590*])

* Installing {rh-rhacm} 2.4.2 on a bare-metal host causes the Kubernetes API Server operator to report the following error:
+
[source,terminal]
----
MutatingAdmissionWebhookConfigurationDegraded: managedclustermutators.admission.cluster.open-cluster-management.io: x509: certificate signed by unknown authority
----
+
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2034913[*BZ#2034913*])

* Installing {rh-rhacm} 2.5.0 in the `ocm` namespace causes the `infrastructure-operator` pod to fail due to insufficient privileges.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2046554[*BZ#2046554*])

* {product-title} limits object names to 63 characters. If a policy name defined in a `PolicyGenTemplate` CR approaches this limit, the Topology Aware Lifecycle Operator cannot create child policies. When this occurs, the parent policy will remain in a `NonCompliant` state.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2057209[*BZ#2057209*])

* In the default ZTP Argo CD configuration, cluster names cannot begin with `ztp`. Using names starting with `ztp` for clusters deployed with Zero Touch Provisioning (ZTP) results in provisioning not completing. As a workaround, ensure that either cluster names do not start with `ztp`, or adjust the Argo CD policy application namespace to a pattern that excludes the names of your clusters but still matches your policy namespace. For example, if your cluster names start with `ztp`, change the pattern in the Argo CD policy app configuration to something different, like `ztp-`.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2049154[*BZ#2049154*])
...
* In {product-title} 4.10, `linxptp` daemon usage has more than doubled since earlier measurements in version 4.9. For an ordinary clock PTP configuration using the Intel e810 NIC, CPU usage approaches 45 mc. No faults are recorded in the logs. These results are found with the following NIC configuration:
+
[source,terminal]
----
NIC
...
----
+
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2050994[*BZ#2050994*])

* During a spoke cluster upgrade from 4.9 to 4.10, with heavy workload running, the `kube-apiserver` pod can take longer than the expected time to start. As a result, the upgrade does not complete and the `kube-apiserver` rolls back to the previous version.
(link:https://bugzilla.redhat.com/show_bug.cgi?id=2064024[*BZ#2064024*])
