...
The conversion to the new OVN Kubernetes interconnect multi-zone architecture can lead to packet drops that cause network outages of up to 300ms per node. 
...
* On clusters with the SR-IOV Network Operator installed and configured, pods with a secondary interface of SRI-OV VF fail to create a pod sandbox and will not function. (link:https://issues.redhat.com/browse/OCPBUGS-38091[*OCPBUGS-38091*])
