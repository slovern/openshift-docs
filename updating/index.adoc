...
xref:../updating/updating-clusters-sro[Updating a cluster that includes the SRO]: When updating a cluster that includes the SRO, it is important to consider whether the new kernel module version will be compatible with the kernel modules currently loaded by the SRO. You can run a preflight check to confirm that the SRO will be able to upgrade the kernel modules. For more information, see the following:

* xref:../updating/updating-clusters-sro#update-upgrading-sro[Running a preflight check for the SRO]
