...
Using hub templates in site `PolicyGenTemplate` (PGT) CRs means that you do not need to create single-site `PolicyGenTemplate` CRs for each site.
...
Each cluster should have a label corresponding to the group(s) the cluster is in. 
If the configuration values for each group are kept in different `ConfigMap` CRs, you require just one group `PolicyGenTemplate` CR using hub templates, and the cluster’s name and labels, to retrieve the values for each cluster.
...
+
.. Create `group-hardware-types-configmap`, a `ConfigMap` to hold the hardware specific configuration. For example:
+
[source,yaml]
----
---
...
.. Create `group-zones-configmap`, a `ConfigMap` to hold the regional configuration. For example:
+
[source,yaml]
----
---
...
.. Create `site-data-configmap`, a `ConfigMap` to hold the site specific configuration. For example:
+
[source,yaml]
----
---
...
[NOTE]
====
Each `ConfigMap` must be in the same namespace as the policy to be generated from the group `PolicyGenTemplate` CR that is using hub templates.
====
